...
export function crear({ nota = '', plantilla = null, dias = DIAS_VALIDEZ } = {}) {
  ...
  obtenerDB()
    .prepare(
      `INSERT INTO invitations (token, nota, plantilla, created_at, expires_at)
       VALUES (?, ?, ?, ?, ?)`
    )
    .run(token, String(nota).slice(0, 120), plantilla, creada.toISOString(), expira.toISOString());

  return { token, nota, plantilla, expiresAt: expira.toISOString() };
}
